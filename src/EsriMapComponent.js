...
const EsriMapComponent = () => {
  ...
  const layerListWidgetRef = useRef(null);
  ...
  const initializeMap = async () => {
    try {
      ...
      const routeLayerWidget = new RouteLayer();
      const routeParams = new RouteParameters({
        apiKey: 'AAPK3d737cc3995b41de949427b52d04f894Ujr4K1pyTeeqmvTK161wvqE6yVRI_bv7MLOWyOrKzXhG9DaHWeR4T-cXQfIGgmEW',
      ...
      mapView.current.ui.add(locateWidget, 'top-left');
      ...
      mapView.current.on("double-click", (event) => {
        ...
        if (routeParams.stops.features.length >= 2) {
          solve(routeUrl, routeParams).then(showRoute);
        }
      });
      ...
      const formatDataForFirebase = (data) => {
        return data.map((feature) => {
          return {
            ...
            sport: feature.attributes.description,
            noOfFields: feature.attributes.No_fields,
            facilities: feature.attributes.Facilities,
            address: feature.attributes.Address,
            phoneNumber: feature.attributes.PhoneNumber,
            pricePerHour: feature.attributes.Price
          };
        });
      };
      ...
      const createTennisLayer = async (visibility) => {
        ...
        return new FeatureLayer({
          ...
          outFields: ['*'],
          popupTemplate: popupTennisFields,
          renderer: {
            type: 'simple',
            symbol: {
              type: 'picture-marker',
      ...
      const tennisLayer = await createTennisLayer(false);
      map.add(tennisLayer);
      ...
      const createBasketballLayer = async (visibility) => {
        ...
        return new FeatureLayer({
          ...
          outFields: ['*'],
          popupTemplate: popupBasketballFields,
          renderer: {
            type: 'simple',
            symbol: {
              type: 'picture-marker',
      ...
      const favouriteTennisFieldsLayer = new FeatureLayer({
        url: 'https://services6.arcgis.com/3T4q3twraXHKJdR1/ArcGIS/rest/services/Tenis_Layer/FeatureServer',
        id: 'favouriteTennisLayerId',
        title: 'Terenuri de tenis favorite',
        outFields: ['*'],
        visible: false,
        definitionExpression: '',
        popupTemplate: popupTennisFields,
      ...
      onValue(tennisFieldsFavouriteRef, (snapshot) => {
        ...
        // Create a query to filter the `footballLayer` based on these IDs
        const query = tennisLayer.createQuery();
        query.where = 'id IN (' + favoriteFieldIds.join(', ') + ')';
      ...
      const favouriteBasketballFieldsLayer = new FeatureLayer({
        url: 'https://services6.arcgis.com/3T4q3twraXHKJdR1/arcgis/rest/services/Basketball_Layer/FeatureServer',
        id: 'favouriteBasketballLayerId',
        title: 'Terenuri de baschet favorite',
        outFields: ['*'],
        visible: false,
        definitionExpression: '',
        popupTemplate: popupBasketballFields,
      ...
      onValue(basketballFieldsFavouriteRef, (snapshot) => {
        ...
        // Create a query to filter the `footballLayer` based on these IDs
        const query = basketballLayer.createQuery();
        query.where = 'id IN (' + favoriteFieldIds.join(', ') + ')';
      ...
      if (!layerListWidgetRef.current) {
        ...
        mapView.current.ui.add(layerListWidget, "top-right");
        layerListWidgetRef.current = layerListWidget;
      }
